    ...
    for (const aud of auditorias) {
      ...
      try {
        ...
        switch (aud.tabla_afectada) {
          ...
          case 'asistencias':
            if (aud.operacion === 'INSERT') {
              // Obtener la fecha de asistencia
              let fechaAsistencia = datosNuevos.fecha;
              let estadoAsistencia = datosNuevos.estado;
              let observacionesAsistencia = datosNuevos.observaciones;

              // Si no tenemos la fecha, consultar la BD
              if (!fechaAsistencia) {
                try {
                  const [asistenciaInfo] = await pool.execute(
                    'SELECT estado, fecha, observaciones FROM asistencias WHERE id_asistencia = ?',
                    [aud.id_registro]
                  );
                  if (asistenciaInfo.length > 0) {
                    fechaAsistencia = asistenciaInfo[0].fecha;
                    estadoAsistencia = asistenciaInfo[0].estado;
                    observacionesAsistencia = asistenciaInfo[0].observaciones;
                  }
                } catch (e) {
                  console.error('Error al consultar asistencia:', e);
                }
              }

              // No mostrar el estado en la descripción, solo la fecha
              descripcion = `Registró asistencia - Fecha: ${fechaAsistencia || 'No especificada'}`;
              detalles = {
              ...
              const fechaAsistencia = datosNuevos.fecha || datosAnteriores.fecha;

              if (datosNuevos.justificacion) {
                descripcion = `Agregó justificación de asistencia - Fecha: ${fechaAsistencia}`;
              } else {
                descripcion = `Actualizó registro de asistencia - Fecha: ${fechaAsistencia}`;
              }
              detalles = {
                fecha: fechaAsistencia,
                estado_anterior: datosAnteriores.estado,
