...
exports.listTiposCursos = async (req, res) => {
  try {
    ...
    let sql = `SELECT 
      id_tipo_curso, nombre, descripcion, duracion_meses, precio_base, 
      modalidad_pago, numero_clases, precio_por_clase, matricula_incluye_primera_clase,
      estado, card_key 
      FROM tipos_cursos WHERE 1=1`;
    const params = [];
...
exports.createTipoCurso = async (req, res) => {
  try {
    const { 
      nombre, descripcion, duracion_meses, precio_base, 
      modalidad_pago, numero_clases, precio_por_clase, matricula_incluye_primera_clase,
      estado, card_key 
    } = req.body;
    ...
    const sql = `INSERT INTO tipos_cursos (
      nombre, descripcion, duracion_meses, precio_base, 
      modalidad_pago, numero_clases, precio_por_clase, matricula_incluye_primera_clase,
      estado, card_key
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    ...
    const params = [
      ...
      matricula_incluye_primera_clase ?? true,
      estado || 'activo', 
      computedKey || null
    ];
    ...
    const [rows] = await pool.execute(`SELECT 
      ...
      modalidad_pago, numero_clases, precio_por_clase, matricula_incluye_primera_clase,
      estado, card_key 
      FROM tipos_cursos WHERE id_tipo_curso = ?`, [result.insertId]);
...
exports.updateTipoCurso = async (req, res) => {
  try {
    ...
    const sql = `UPDATE tipos_cursos SET 
      nombre = COALESCE(?, nombre), 
      descripcion = COALESCE(?, descripcion), 
      duracion_meses = COALESCE(?, duracion_meses), 
      precio_base = COALESCE(?, precio_base),
      modalidad_pago = COALESCE(?, modalidad_pago),
      numero_clases = COALESCE(?, numero_clases),
      precio_por_clase = COALESCE(?, precio_por_clase),
      matricula_incluye_primera_clase = COALESCE(?, matricula_incluye_primera_clase),
    ...
    const params = [
      ...
      numero_clases ?? null,
      precio_por_clase ?? null,
      matricula_incluye_primera_clase ?? null,
      estado ?? null, 
      newCardKey ?? null, 
      id
    ];
    ...
    const [rows] = await pool.execute(`SELECT 
      ...
      modalidad_pago, numero_clases, precio_por_clase, matricula_incluye_primera_clase,
      estado, card_key 
      FROM tipos_cursos WHERE id_tipo_curso = ?`, [id]);
